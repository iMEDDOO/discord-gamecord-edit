...
module.exports = class TicTacToe extends approve {
  constructor(options = {}) {
    ...
    if (!options.requestopponent) options.requestopponent = '{opponent}';
  ...
  async TicTacToeGame(msg) {
    ...
    .setDescription(`${this.getTurnMessage()}`)

    await msg.edit({ content: `${this.message.author} vs ${this.opponent}`, embeds: [embed], components: this.getComponents() });
  ...
  handleButtons(msg) {
    ...
    collector.on('collect', async btn => {
      ...
      return await msg.edit({ content: `${this.message.author} vs ${this.opponent}`, embeds: [embed], components: this.getComponents() });
    })
  ...
  async gameOver(msg, result) {
    ...
    const embed = new EmbedBuilder()
    .setColor(this.options.embed.color)
    .setDescription(`${this.getTurnMessage(result + 'Message')}`)

    return await msg.edit({ content: `${this.message.author} vs ${this.opponent}`, embeds: [embed], components: disableButtons(this.getComponents()) });
  }
